...
from vllm.model_executor.layers.rejection_sampler import RejectionSampler, SpecGRPOSampler
...
class SpecDecodeWorker(LoRANotSupportedWorkerBase):
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def create_worker(
        ...
        elif draft_token_acceptance_method == "SpecGRPOSampler":
            spec_decode_sampler = SpecGRPOSampler()
    ...
    def _run_speculative_decoding_step(
        ...
        if self._vocab_size_delta > 0:
            ...
            # vocab_size = 151665  # 有效的 vocab size
            # probs = proposals.proposal_probs

            # # 1. 将 vocab_size 之后的概率置 0
            # probs[..., vocab_size:] = 0.0

            # # 2. 重新归一化（避免除 0，加一个很小的 eps）
            # denom = probs.sum(dim=-1, keepdim=True) + 1e-8
            # probs /= denom

            # proposals.proposal_probs = probs
