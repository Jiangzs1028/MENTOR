...
class SpecGRPOSampler(SpecDecodeStochasticBaseSampler):
    ...
    def _batch_modified_rejection_sampling(
        ...
        Vt = target_probs.size(-1)
        Vd = draft_probs.size(-1)

        assert Vt == Vd, f"vocab size mismatch: target {Vt}, draft {Vd}"
        assert draft_token_ids.dtype == torch.long, draft_token_ids.dtype
        assert draft_token_ids.min() >= 0, draft_token_ids.min().item()
        assert draft_token_ids.max() < Vt, (draft_token_ids.max().item(), Vt)

        # draft_entropy: [batch_size, k]
        draft_entropy = -torch.sum(draft_probs * torch.log(draft_probs + 1e-8), dim=-1)

        # 计算 scaling 系数 α = min(1, entropy / threshold)
        scaling_factor = torch.clamp(draft_entropy / self.entropy_threshold, max=1.0)  # [batch_size, k]

        # reshape for broadcasting: [batch_size, k, 1]
        scaling_factor = scaling_factor.unsqueeze(-1)

        # modified_probs
        target_probs = draft_probs + (target_probs - draft_probs) * scaling_factor
    ...
    def _get_accepted(
        ...
        are accepted.
        """
        # if (draft_token_ids >= 151665).any():
        #     raise ValueError("Invalid token id 151665 detected in draft_token_ids")
        batch_size, k, _ = draft_probs.shape
