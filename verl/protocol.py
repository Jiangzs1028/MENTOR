...
def overwrite_union_tensor_dict(tensor_dict1: TensorDict, tensor_dict2: TensorDict) -> TensorDict:
    """Union two tensordicts. 
    If a key exists in both, use the value from tensor_dict2.
    """
    if tensor_dict1.batch_size != tensor_dict2.batch_size:
        raise ValueError(
            f"Two tensor dict must have identical batch size. "
            f"Got {tensor_dict1.batch_size} and {tensor_dict2.batch_size}"
        )
...
class DataProto:
    ...
    def union_overwrite(self, other: "DataProto") -> "DataProto":
        ...
        """
        self.batch = overwrite_union_tensor_dict(self.batch, other.batch)
        self.non_tensor_batch = overwrite_union_numpy_dict(self.non_tensor_batch, other.non_tensor_batch)
    ...
    @staticmethod
    def concat_by_uid(batch: "DataProto", other_batch: "DataProto", uid_key: str = "uid") -> "DataProto":
        """按照 uid 顺序拼接：每个 uid 的 old_batch 样本在前，other_batch 样本紧随其后"""
        uids1 = batch.non_tensor_batch[uid_key].tolist()
        uids2 = other_batch.non_tensor_batch[uid_key].tolist()
        if set(uids1) != set(uids2):
            raise ValueError("uid sets differ between the two batches.")

        # 按照第一个 batch 中的出现顺序作为全局顺序
        uid_order = list(dict.fromkeys(uids1))

        # 辅助函数：找出相同 uid 的连续段 slice
        def build_slices(uids: list) -> dict[str, list[slice]]:
            m = {}
            i = 0
            while i < len(uids):
                u = uids[i]
                j = i + 1
                while j < len(uids) and uids[j] == u:
                    j += 1
                m.setdefault(u, []).append(slice(i, j))
                i = j
            return m

        slices1 = build_slices(uids1)
        slices2 = build_slices(uids2)

        pieces = []
        for uid in uid_order:
            for sl in slices1[uid]:
                pieces.append(batch[sl])         # 直接切片返回 DataProto
            for sl in slices2[uid]:
                pieces.append(other_batch[sl])   # 同上
