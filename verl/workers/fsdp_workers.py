...
class FSDPWorker(Worker):
    def __init__(
        ...
        # if self.config.rollout.use_spec:
        #     self.model_state_dice
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                # if self.config.actor.use_spec:
                metrics, q80, q_mean = self.actor.update_policy(data=data)
                # else:
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def prepare_for_spec(self,q80):
        self.q80 = q80

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def prepare_spec_rollout_engine(self, prompts: DataProto):
        print_gpu_memory_usage("加载spec前显存")
        model_state = self.rollout_sharding_manager._get_actor_weight()
        spec_batch = self.rollout.begin_spec_inference_engine(model_state, prompts, self.q80)
        del model_state
        torch.cuda.empty_cache()
        print_gpu_memory_usage("终止spec后显存")
