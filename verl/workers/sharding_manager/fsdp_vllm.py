...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def _make_weight_dict(self,
        ...
        for name, tensor in actor_weights.items():
            if self.world_size != 1 and hasattr(tensor, "full_tensor"):
                tensor = tensor.full_tensor()
            model_state[name] = tensor.cpu().to(torch.bfloat16)
        return model_state
    ...
    def _get_actor_weight(self):
        if self.use_param_offload:
            load_fsdp_model(self.module)
        # print_gpu_memory_usage("加载fsdp后的显存")
        actor_weights = get_model_state_dict(self.module)
        actor_weights = self._rename_weight_keys(actor_weights, self.module._fsdp_wrapped_module)
        ...
        actor_weights = self._make_weight_dict(actor_weights)
        torch.cuda.empty_cache()
