...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, expert_cap=1, expert_weight=1, gamma: float = 1.0, lam: float = 1.0, ):
    ...
    adv_inputs = {
        ...
        "gamma": gamma,
        "lam": lam,
        "is_expert": data.non_tensor_batch["is_expert"],
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if config.worker.rollout.use_mentor:
            self.config.worker.actor.use_mentor = True
    ...
    def _maybe_log_train_generations(
        self, inputs: list[str], outputs: list[str], labels: list[str], scores: list[float], sample_is_expert
    ) -> None:
        """Log a table of validation samples"""
        # if self.config.trainer.val_generations_to_log <= 0:
        #     return
        ...
        # Create tuples of (input, output, score) and sort by input text
        samples = list(zip(inputs, outputs, labels, scores, sample_is_expert))
        experts = [s for s in samples if float(s[4]) == 1]
        non_experts = [s for s in samples if float(s[4]) == 0]


        # Use fixed random seed for deterministic shuffling
        rng = np.random.RandomState(42)
        # rng.shuffle(experts)
        experts = sorted(experts, key=lambda s: s[3], reverse=True)
        rng.shuffle(non_experts)

        picked_experts = experts[:3]
        picked_non_experts = non_experts[:3]

        selected = picked_experts + picked_non_experts

        self.logger.log_generation_train(selected, self.global_step)
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        if self.config.worker.rollout.use_mentor:
            self.config.worker.rollout.n -= self.config.worker.rollout.n_mixedpolicy
        batch = None
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            self.global_step += 1
            expert_weight = 0 if self.global_step >= self.config.algorithm.expert_weight_decay_totalstep else self.config.algorithm.expert_weight * 0.5 * (1 + math.cos(math.pi * self.global_step / self.config.algorithm.expert_weight_decay_totalstep)) # max(1 - self.global_step/200, 0)
            ...
            with timer("step", timing_raw):
                ...
                with timer("gen", timing_raw):
                    ...
                    if self.config.worker.rollout.use_mentor and expert_weight > 0:
                        spec_batch = deepcopy(batch[::self.config.worker.rollout.n-self.config.worker.rollout.n_mixedpolicy])
                        self.actor_rollout_ref_wg.prepare_for_spec(q80)
                        gen_mixedpolicy_batch = self.actor_rollout_ref_wg.prepare_spec_rollout_engine(spec_batch) 


                        spec_batch = spec_batch.repeat(repeat_times=self.config.worker.rollout.n_mixedpolicy, interleave=True)
                        spec_batch = spec_batch.union_overwrite(gen_mixedpolicy_batch)

                        batch = DataProto.concat_by_uid(batch, spec_batch)
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        reward_tensor, reward_metrics = ray.get(reward_ref) # 这里为什么有两个
                    ...
                    if self.config.worker.rollout.use_mentor:
                        self.config.algorithm.adv_estimator = "mixedpolicy_grpo"
                    ...
                    batch = compute_advantage(
                        ...
                        expert_cap = self.config.algorithm.expert_cap,
                        expert_weight = expert_weight
                    )
                ...
                if self.config.trainer.critic_warmup <= self.global_step:
                    ...
                    if self.config.worker.rollout.use_mentor:
                        if "actor/q80" in metrics:
                            q80 = metrics["actor/q80"]
                        else:
                            print("[Warn] metrics no 'actor/q80', unchange:", q80)
                        metrics["actor/q80"] = q80
                        print(">>>q80:", q80)
                
                
                if self.config.worker.rollout.use_mentor and self.global_step % 2 == 0:
                    sample_inputs, sample_outputs, sample_labels, sample_scores, sample_is_expert = [], [], [], [], []
                    ...
                    sample_inputs.extend(input_texts)
                    sample_outputs.extend(output_texts)
                    sample_labels.extend(batch.non_tensor_batch["ground_truth"].tolist())
                    sample_is_expert.extend(batch.non_tensor_batch["is_expert"].tolist())
                    sample_scores.extend(scores)
                    self._maybe_log_train_generations(sample_inputs, sample_outputs, sample_labels, sample_scores, sample_is_expert)
