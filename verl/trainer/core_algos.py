...
class AdvantageEstimator(str, Enum):
    ...
    REMAX = "remax"
    RLOO = "rloo"
    Mixedpolicy_GRPO = 'mixedpolicy_grpo'
...
@register_adv_estimator(AdvantageEstimator.Mixedpolicy_GRPO)
def compute_mixedpolicy_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, is_expert, expert_cap, expert_weight, eps: float = 1e-6, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for Mixed-policy GRPO:
      - Non-expert samples: same normalization as GRPO within the same uid (index).
      - Expert samples: advantage is set to 1 over response tokens.

    Args:
        token_level_rewards: (bs, response_length)
        response_mask:       (bs, response_length) — 1 for response tokens, 0 for others
        index:               (bs,) — uid for grouping (e.g., the same prompt/query)
        eps:                 float — avoid division by zero
        is_expert:           (bs,) — bool/int tensor, True is mixed_policy rollout

    Returns:
        advantages: (bs, response_length)
        returns:    (bs, response_length)
    """
    print("[AdvantageEstimator]: Mixed-policy GRPO")
    if not torch.is_tensor(is_expert):
        is_expert = torch.tensor(is_expert, device=token_level_rewards.device)
    is_expert = is_expert.to(dtype=torch.bool, device=token_level_rewards.device)
    id2mean_nonexp, id2std_nonexp = {}, {}
    # (bs,)
    scores = token_level_rewards.sum(dim=-1)
    bsz = scores.shape[0]

    id2scores_nonexp = defaultdict(list)
    for i in range(bsz):
        if not is_expert[i]:
            id2scores_nonexp[index[i]].append(scores[i])

    for idx in id2scores_nonexp:
        assert len(id2scores_nonexp[idx]) > 1, "GRPO needs rollout.n > 1."
        id2mean_nonexp[idx] = torch.mean(torch.tensor(id2scores_nonexp[idx]))
        id2std_nonexp[idx] = torch.std(torch.tensor(id2scores_nonexp[idx]))

    for i in range(bsz):
        if not is_expert[i]:
            scores[i] = (scores[i] - id2mean_nonexp[index[i]]) / (id2std_nonexp[index[i]] + eps)
        else:
            scores[i] = torch.clamp_min(scores[i] - id2mean_nonexp[index[i]], 0.0) 


    id2expert_idx = defaultdict(list)
    for i in range(bsz):
        if is_expert[i]:
            id2expert_idx[index[i]].append(i)

    for uid, idxs in id2expert_idx.items():
        exp_scores = scores[idxs]
        total = exp_scores.sum()
        if total > expert_cap:  
            scores[idxs] = exp_scores / total * expert_cap
        scores[idxs] = scores[idxs] * expert_weight
