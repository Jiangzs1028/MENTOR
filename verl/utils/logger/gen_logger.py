...
class GenerationLogger(ABC):
    ...
    @abstractmethod
    def log_train(self, samples: List[Tuple[str, str, str, float, bool]], step: int) -> None: ...
...
class ConsoleGenerationLogger(GenerationLogger):
    ...
    def log_train(self, samples: List[Tuple[str, str, str, float]], step: int) -> None:
        for inp, out, lab, score, is_expert in samples:
            print(f"[prompt] {inp}\n[output] {out}\n[ground_truth] {lab}\n[score] {score}\n[is_expert] {is_expert}\n")
...
class SwanlabGenerationLogger(GenerationLogger):
    ...
    def log_train(self, samples: List[Tuple[str, str, str, float, bool]], step: int) -> None:
        ...
        for i, sample in enumerate(samples):
            row_text = "\n\n---\n\n".join(
                (f"input: {sample[0]}", f"output: {sample[1]}", f"label: {sample[2]}", f"score: {sample[3]}", f"is_expert: {sample[4]}")
            )
            swanlab_text_list.append(swanlab.Text(row_text, caption=f"sample {i + 1}"))

        swanlab.log({"train/generations": swanlab_text_list}, step=step)
